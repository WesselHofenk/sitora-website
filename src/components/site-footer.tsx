    ...
    <footer className="bg-[#040b14] pb-10 pt-20 text-slate-300 sm:pt-28">
      <div className="mx-auto max-w-[86rem] px-5 sm:px-8 lg:px-10">
        <div className="grid gap-12 border-b border-white/10 pb-16 lg:grid-cols-[1.35fr_.65fr] lg:items-end">
          <div><Logo light /><p className="mt-8 max-w-4xl text-3xl font-black leading-[1.08] tracking-[-.045em] text-white sm:text-4xl lg:text-5xl">Professionele websites voor bedrijven die hun vak verstaan.</p></div>
        ...
        <div className="grid gap-12 py-16 sm:grid-cols-2 lg:grid-cols-[1.15fr_1fr_1fr_.85fr]">
          <div><p className="max-w-sm leading-7 text-slate-400">Sitora ontwerpt duidelijke, conversiegerichte websites voor vak- en bouwbedrijven in Nederland en België.</p><div className="mt-7 space-y-2 text-sm">{contact.phoneHref ? <a className="flex min-h-11 items-center gap-3 hover:text-white" href={`tel:${contact.phoneHref}`}><Phone className="size-4 text-orange-400" />{contact.phoneDisplay}</a> : null}<a className="flex min-h-11 items-center gap-3 hover:text-white" href={`mailto:${contact.email}`}><Mail className="size-4 text-orange-400" />{contact.email}</a><p className="flex min-h-11 items-center gap-3"><MapPin className="size-4 text-orange-400" />{contact.location}</p></div></div>
          <div><h2 className="text-xs font-black uppercase tracking-[.18em] text-orange-400">Voor vakbedrijven</h2><ul className="mt-5 space-y-1">{sectors.map((sector) => <li key={sector.slug}><Link className="inline-flex min-h-10 items-center text-sm hover:text-white" href={`/${sector.slug}`}>Websites voor {sector.plural}</Link></li>)}</ul></div>
