...
export function TrustStrip() {
  ...
  return <section aria-label="Feiten over Sitora" className="bg-white py-6"><div className="mx-auto grid max-w-[86rem] gap-3 px-5 sm:grid-cols-2 sm:px-8 lg:grid-cols-4 lg:px-10">{facts.map(([label, value]) => <div key={label} className="rounded-xl bg-slate-100 p-4"><p className="text-[10px] font-black uppercase tracking-wider text-orange-800">{label}</p><p className="mt-2 text-sm font-black text-slate-900">{value}</p></div>)}</div></section>;
}

export function FounderTrust() {
  return <section className="bg-white py-16 sm:py-20"><div className="mx-auto grid max-w-[86rem] gap-8 px-5 sm:px-8 lg:grid-cols-[.7fr_1.3fr] lg:items-center lg:px-10"><div className="grid min-h-64 place-items-center rounded-3xl bg-[#07111f] p-8 text-center text-white"><span className="grid size-20 place-items-center rounded-full bg-orange-700 text-2xl font-black" aria-hidden="true">WH</span><p className="mt-5 font-black">{business.ownerName}</p><p className="mt-2 text-sm text-slate-300">Eigenaar van Sitora</p></div><div><SectionHeading eyebrow="Persoonlijk verantwoordelijk" title="Rechtstreeks contact met degene die je website uitwerkt" description={`${business.ownerName} is het vaste aanspreekpunt voor intake, keuzes, feedback en oplevering. Zo blijven afspraken, scope en vervolgstappen overzichtelijk.`} /><ul className="mt-7 grid gap-3 sm:grid-cols-2">{["Vooraf duidelijk wat binnen de scope valt", "Feedback op afgesproken momenten", "Geen verplicht onderhoudsabonnement", "Externe kosten apart benoemd"].map((item) => <li key={item} className="rounded-xl bg-slate-100 p-4 text-sm font-bold text-slate-800">{item}</li>)}</ul><ButtonLink href="/over-sitora" variant="secondary" className="mt-7">Lees over Sitora</ButtonLink></div></div></section>;
}
...
export function RevisionPromise() {
  ...
  return <section className="bg-orange-500 py-16 text-[#07111f] sm:py-20 lg:py-24"><div className="mx-auto max-w-[86rem] px-5 sm:px-8 lg:px-10"><div className="grid gap-8 lg:grid-cols-[1.15fr_.85fr] lg:items-end"><div><p className="text-xs font-black uppercase tracking-[.2em]">Duidelijk geregeld</p><h2 className="mt-5 max-w-4xl text-balance text-[clamp(2.5rem,7vw,4.5rem)] font-black leading-[.98] tracking-[-.06em]">Maatwerk zonder onduidelijke kosten of vaste verplichtingen.</h2></div><p className="max-w-xl text-lg leading-8 text-slate-950">Je kiest een helder pakket. Wat buiten de scope valt, bespreken en prijzen we altijd voordat het werk begint.</p></div><div className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{trust.map((item, index) => <div key={item} className="flex min-h-20 items-center gap-3 rounded-xl bg-[#07111f]/8 p-5"><span className="text-xs font-black">0{index + 1}</span><strong>{item}</strong></div>)}</div><ButtonLink href="/pakketten" variant="secondary" className="mt-8 bg-white/80">Bekijk alle prijzen</ButtonLink></div></section>;
}
