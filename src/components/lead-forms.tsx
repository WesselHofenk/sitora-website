  ...
  return (
    <form onSubmit={submit} noValidate data-form-type="compact" className="rounded-[1.25rem] border border-white/10 bg-[#f6f3ed] p-6 text-slate-950 sm:p-9" aria-describedby={formError ? "compact-form-error" : undefined}>
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="grid gap-2 text-sm font-black text-slate-800">Naam <span aria-hidden="true" className="text-orange-800">*</span><input className={inputClass} name="name" autoComplete="name" maxLength={100} required aria-invalid={!!errors.name} /><FieldError message={errors.name} /></label>
        ...
        <label className="grid gap-2 text-sm font-black text-slate-800">E-mailadres <span aria-hidden="true" className="text-orange-800">*</span><input className={inputClass} name="email" type="email" autoComplete="email" maxLength={180} required aria-invalid={!!errors.email} /><FieldError message={errors.email} /></label>
        ...
        <label className="grid gap-2 text-sm font-black text-slate-800">Branche <span aria-hidden="true" className="text-orange-800">*</span><select className={inputClass} name="industry" defaultValue="" required aria-invalid={!!errors.industry}><option value="" disabled>Kies je branche</option>{industries.map((industry) => <option key={industry}>{industry}</option>)}</select><FieldError message={errors.industry} /></label>
        <label className="grid gap-2 text-sm font-black text-slate-800">Huidige website <span aria-hidden="true" className="text-orange-800">*</span><select className={inputClass} name="currentWebsite" defaultValue="" required aria-invalid={!!errors.currentWebsite}><option value="" disabled>Kies een optie</option><option>Ik heb nog geen website</option><option>Ik heb al een website</option></select><FieldError message={errors.currentWebsite} /></label>
        <label className="grid gap-2 text-sm font-black text-slate-800 sm:col-span-2">Waarmee kunnen we helpen? <span aria-hidden="true" className="text-orange-800">*</span><select key={offerDefault} className={inputClass} name="package" defaultValue={offerDefault} required aria-invalid={!!errors.package}>{offerOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select><FieldError message={errors.package} /></label>
      </div>
      <label className="mt-5 grid gap-2 text-sm font-black text-slate-800">Bericht <span aria-hidden="true" className="text-orange-800">*</span><textarea className={textareaClass} name="message" maxLength={2000} required aria-invalid={!!errors.message} placeholder="Vertel kort waar je hulp bij zoekt." /><FieldError message={errors.message} /></label>
...
export function DetailedAdviceForm({ initialOffer = "overig" }: { initialOffer?: string }) {
  ...
  return (
    <form onSubmit={submit} noValidate data-form-type="detailed" className="rounded-[1.25rem] border border-slate-900/15 bg-white p-6 sm:p-9" aria-describedby={formError ? "detailed-form-error" : undefined}>
      ...
      <button disabled={loading || submitted} type="submit" className="mt-6 inline-flex min-h-13 w-full items-center justify-center gap-2 rounded-full bg-orange-700 px-5 py-3 font-black text-white transition-[background-color,transform] duration-150 hover:bg-orange-800 active:scale-[.98] disabled:cursor-not-allowed disabled:opacity-60">{loading ? <><LoaderCircle className="size-4 animate-spin" />Aanvraag versturen…</> : submitted ? <><Check className="size-4" />Aanvraag ontvangen</> : <>Ontvang gratis websiteadvies <Send className="size-4" /></>}</button>
      <p className="mt-4 text-center text-xs text-slate-700">Versturen is vrijblijvend. Na succesvolle verzending nemen we contact met je op.</p>
    </form>
  );
}
