...
function AudienceSection({ sector }: { sector: Sector }) {
  return <section className="py-16 sm:py-20"><div className="mx-auto grid max-w-[86rem] gap-10 px-5 sm:px-8 lg:grid-cols-2 lg:items-start lg:px-10"><div><SectionHeading eyebrow="Bezoekers en hun vraag" title={`Wat bezoekers van ${sector.plural.toLowerCase()} willen weten`} description={sector.visitorIntent} /><div className="mt-7 flex flex-wrap gap-2">{sector.customerTypes.map((type) => <span key={type} className="rounded-full bg-white px-4 py-2 text-sm font-bold shadow-sm">{type}</span>)}</div></div><div className="rounded-3xl bg-slate-100 p-7 sm:p-9"><p className="text-sm font-black text-orange-800">Gewenste vervolgstap</p><p className="mt-3 text-2xl font-black tracking-[-.03em] text-slate-950">{sector.desiredAction}</p><h2 className="mt-8 text-lg font-black">Drempels die eerst moeten worden weggenomen</h2><ul className="mt-4 space-y-3">{sector.trustBarriers.map((barrier) => <li key={barrier} className="flex gap-3 text-slate-700"><CheckCircle2 className="mt-0.5 size-5 shrink-0 text-orange-800" aria-hidden="true" />{barrier}</li>)}</ul></div></div></section>;
}
...
export function SectorPage({ sector }: { sector: Sector }) {
  ...
  return <>
    ...
    <section className="bg-[#f6f3ed] py-10"><div className="mx-auto max-w-[86rem] px-5 sm:px-8 lg:px-10"><p className="text-xs font-black uppercase tracking-[.18em] text-orange-800">Waar de huidige klantreis vaak vastloopt</p><div className="mt-6 grid gap-3 md:grid-cols-3">{sector.problems.map((problem, index) => <div key={problem} className="flex gap-4 rounded-xl bg-white p-5 font-bold text-slate-800"><span className="text-xs text-orange-800">0{index + 1}</span>{problem}</div>)}</div></div></section>
    {orderedSections}
