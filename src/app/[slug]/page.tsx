...
function ChatbotPage() {
  ...
  return <><JsonLd data={structuredData} /><PageHero eyebrow="Chatbot voor je website" title="Een slimme chatbot op je website voor € 149" description="Geef bezoekers snel antwoord op vooraf afgestemde veelgestelde vragen, ook wanneer je zelf niet beschikbaar bent. Eenmalig, exclusief btw en zonder verplicht abonnement."><ButtonLink href={`/contact?dienst=${chatbotOffer.id}#advies`}>Chatbot aanvragen</ButtonLink><ButtonLink href="/website-onderhoud" variant="light">Bekijk websiteonderhoud</ButtonLink></PageHero><section className="bg-[#f6f3ed] py-20 sm:py-24"><div className="mx-auto grid max-w-[86rem] gap-12 px-5 sm:px-8 lg:grid-cols-[1fr_.8fr] lg:items-center lg:px-10"><div><SectionHeading eyebrow="Laagdrempelige bezoekershulp" title="Veelgestelde vragen direct opvangen" description="De chatbot helpt bezoekers met informatie die vooraf met Sitora is afgestemd. Wanneer een antwoord niet betrouwbaar beschikbaar is, blijft persoonlijk contact de juiste vervolgstap." /><ul className="mt-8 grid gap-4 sm:grid-cols-2">{chatbotOffer.features.map((feature) => <li key={feature} className="flex gap-3 rounded-xl border border-slate-200 bg-white p-4 font-bold text-slate-800"><CheckCircle2 className="mt-0.5 size-5 shrink-0 text-emerald-700" aria-hidden="true" />{feature}</li>)}</ul></div><aside className="rounded-3xl bg-[#07111f] p-8 text-white shadow-2xl sm:p-10"><p className="text-sm font-black text-orange-300">{chatbotOffer.name}</p><p className="mt-4 text-6xl font-black tracking-[-.06em]">{chatbotOffer.price}</p><p className="mt-3 text-sm text-slate-200">{chatbotOffer.cadence}</p><p className="mt-7 leading-7 text-slate-200">{chatbotOffer.description}</p><ButtonLink href={`/contact?dienst=${chatbotOffer.id}#advies`} className="mt-8 w-full">Chatbot aanvragen</ButtonLink><p className="mt-5 text-xs leading-5 text-slate-300">Eventuele externe licenties en werkzaamheden buiten de afgesproken installatie worden vooraf besproken.</p></aside></div></section><section className="bg-white py-20 sm:py-24"><div className="mx-auto max-w-7xl px-5 sm:px-8"><SectionHeading center eyebrow="Zo werkt de installatie" title="Van vragenlijst naar chatbot in drie stappen" /><div className="mt-12 grid gap-5 md:grid-cols-3">{steps.map((step) => <article key={step.number} className="rounded-2xl border border-slate-200 bg-[#f6f3ed] p-7"><span className="text-xs font-black text-orange-800">{step.number}</span><h2 className="mt-5 text-2xl font-black">{step.title}</h2><p className="mt-3 leading-7 text-slate-600">{step.text}</p></article>)}</div></div></section><FaqList eyebrow="Vragen over de chatbot" title="Duidelijk over prijs en mogelijkheden" items={chatbotFaqs} /><BottomCta /></>;
}
...
function AboutPage() {
  ...
  return <><PageHero eyebrow="Over Sitora" title={`${business.ownerName} is je vaste aanspreekpunt van intake tot oplevering`} description={`Sitora is een persoonlijk webbedrijf voor mkb, dienstverleners, vakbedrijven en organisaties in ${business.serviceArea}.`}><ButtonLink href="/contact#advies">Maak kennis</ButtonLink></PageHero><section className="py-16 sm:py-20"><div className="mx-auto max-w-7xl px-5 sm:px-8"><div className="grid gap-10 lg:grid-cols-[.7fr_1.3fr] lg:items-start"><aside className="rounded-3xl bg-[#07111f] p-8 text-white"><div className="grid size-20 place-items-center rounded-full bg-orange-700 text-2xl font-black" aria-hidden="true">WH</div><h2 className="mt-6 text-2xl font-black">{business.ownerName}</h2><p className="mt-2 text-slate-300">Eigenaar van Sitora</p><dl className="mt-7 space-y-4 text-sm"><div><dt className="text-slate-300">KvK</dt><dd className="font-bold">{business.chamberOfCommerce}</dd></div><div><dt className="text-slate-300">Werkwijze</dt><dd className="font-bold">{business.workingModel}</dd></div><div><dt className="text-slate-300">Werkgebied</dt><dd className="font-bold">{business.serviceArea}</dd></div></dl></aside><div><SectionHeading eyebrow="Waarom Sitora" title="Een websiteproject hoeft niet onnodig ingewikkeld te zijn" description="Sitora bestaat om bedrijven en organisaties door technische en inhoudelijke keuzes te begeleiden zonder onduidelijke pakketten, vaste onderhoudsverplichtingen of overdreven resultaatclaims." /><p className="mt-6 max-w-3xl leading-8 text-slate-600">Je overlegt rechtstreeks met Wessel over inhoud, ontwerp, techniek en feedback. Wanneer een koppeling, leverancier of specialist buiten de afgesproken scope valt, wordt dat benoemd voordat er kosten of afhankelijkheden ontstaan.</p><div className="mt-9 grid gap-5 md:grid-cols-2">{values.map(({ icon: Icon, title, text }) => <article key={title} className="rounded-2xl bg-slate-100 p-7"><Icon className="size-7 text-orange-800" aria-hidden="true" /><h2 className="mt-5 text-xl font-black">{title}</h2><p className="mt-3 leading-7 text-slate-600">{text}</p></article>)}</div></div></div><div className="mt-12 rounded-3xl bg-white p-8 shadow-sm sm:p-12"><h2 className="text-3xl font-black tracking-[-.04em]">Wat je concreet mag verwachten</h2><ul className="mt-7 grid gap-4 md:grid-cols-2">{["Eerlijk advies over wat je wel en niet nodig hebt", "Een offerte met scope en belangrijke uitsluitingen", "Feedback op afgesproken momenten", "Controle op mobiel, formulieren en technische basis", "Overdracht volgens offerte en na volledige betaling", "Los onderhoud wanneer jij daar behoefte aan hebt"].map((item) => <li key={item} className="flex gap-3"><CheckCircle2 className="mt-0.5 size-5 shrink-0 text-emerald-700" aria-hidden="true" />{item}</li>)}</ul></div></div></section><BottomCta /></>;
}

function StatusPage() {
  return <section className="grid min-h-[60vh] place-items-center bg-slate-100 px-5 py-20"><div className="max-w-2xl rounded-3xl bg-white p-8 text-center shadow-sm sm:p-12"><p className="text-xs font-black uppercase tracking-wider text-orange-800">Aanvraagstatus</p><h1 className="mt-3 text-4xl font-black tracking-[-.045em] text-slate-950">Deze pagina bevestigt geen inzending</h1><p className="mt-5 leading-7 text-slate-600">Na het versturen toont het contactformulier direct een toegankelijke succes- of foutmelding. Open je deze URL rechtstreeks, dan is er geen aanvraag geregistreerd.</p><ButtonLink href="/contact#advies" className="mt-8">Ga naar het contactformulier</ButtonLink></div></section>;
}
